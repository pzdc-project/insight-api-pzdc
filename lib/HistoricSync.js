...
var BAD_GEN_ERROR = 'Bad genesis block. Network mismatch between Insight and pzdcd? Insight is configured for:';
...
function HistoricSync(opts) {
  ...
  this.rpc = new RpcClient(config.pzdcd);
    ...
    self.sync.bDb.getTip(function(err, tip, height) {
      ...
      async.doWhilst(
        ...
        function(err) {
          ...
          var limit = self.network == 'livenet' ? 24 : 100;
          if (d > limit) {
          ...
          if (self.blockChainHeight === blockInfo.height ||
            ...
            oldtip = tip;
            if (!tip)
              throw new Error('Previous blockchain tip was not found on pzdcd. Please reset Insight DB. Tip was:' + tip)
...
HistoricSync.prototype.prepareFileSync = function(opts, next) {
  ...
  if (opts.forceRPC || !config.pzdcd.dataDir ||
    self.height > self.blockChainHeight * PERCENTAGE_TO_START_FROM_RPC) return next();


  try {
    self.blockExtractor = new BlockExtractor(config.pzdcd.dataDir, config.network);
  } catch (e) {
...
HistoricSync.prototype.showSyncStartMessage = function() {
  ...
  info('Got ' + self.height +
    ' blocks in current DB, out of ' + self.blockChainHeight + ' block at pzdcd');

  if (self.blockExtractor) {
    info('pzdcd dataDir configured...importing blocks from .dat files');
