...
TxController.prototype.multitxdetails = function(req, res, next) {
  var self = this,
    txids;
  if(req.body.txids) {
    txids = req.body.txids.split(',');
  } else {
    txids = req.params.txids.split(',');
  }
  async.map(txids, function(txid, next) {
    ...
    res.jsonp(_.zipObject(txids, transactions));
  });
};
